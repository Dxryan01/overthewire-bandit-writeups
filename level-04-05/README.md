...
**Solution**: `file ./*` → `cat ./FILENAME_HERE`

**Alternative**: `for f in ./*; do file "$f"; done` (same result as `file ./*`, but processes files one by one instead of passing them all as a single argument list — useful if you needed to act differently per file)
