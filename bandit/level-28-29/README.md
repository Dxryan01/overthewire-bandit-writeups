...
**Solution**:
1. Clone the repo on your own local machine then navigate to the cloned directory : `git clone ssh://bandit28-git@bandit.labs.overthewire.org:2220/home/bandit28-git/repo` → `cd repo`
2. Explore the commits made to the repo then compare them : `git log` (note the commit hashes shown for the "initial commit" and "add missing data" entries) → `git diff <initial_commit_hash> <"add missing data"_commit_hash>` (replacing each placeholder with the actual hash string from `git log`'s output) 
